...
if(isset($_GET['action']) && isset($_GET['userId'])){
    if($_GET['action'] == "enable" || $_GET['action'] == "disable"){ // PROCESS THE enable & disable button
      ...
      if($admin->changeAccountStatus($userId)){
        ...
        $redirect = $url."/admin/users?".http_build_query($data, '', '&');
      }
      else{
        $data = array(
          'p' => "users",
  ...
  <?php
      if(isset($_GET['action']) && isset($_GET['userId'])){
        ...
        ?>
  <h2>Edit the User:</h2>
   <form action="/admin/users?p=users&action=update&userId=<?php echo $user['id'] ?>" method="post">
  <div class="row">
  ...
  ?>
      ...
      <div class="row">
        <table class="table">
          <tr>
            <th>Name</th><th>Username</th><th>Email Adress</th><th>Mobile</th><th>Status</th><th>Is Admin</th><th>Account Options</th>
          </tr>
          <?php
          // PULL THE USERS LIST
          ...
          if(is_array($users)){
            ...
            foreach($users as $user){ ?>
              ...
              <a class="btn btn-sm btn-info" href="/admin/users?p=users&action=edit&userId=<?php echo $user['id'] ?>" role="button">Edit</a>
              <a class="btn btn-sm btn-danger" href="#" role="button">Remove</a>
              <?php
              if($user['isActive']){ ?>
                <a class="btn btn-sm btn-warning" href="/admin/users?p=users&action=disable&userId=<?php echo $user['id'] ?>" role="button">Disable</a>
              <?php }else{ ?>
                <a class="btn btn-sm btn-success" href="/admin/users?p=users&action=enable&userId=<?php echo $user['id'] ?>" role="button">Enable</a>
              <?php } ?>
